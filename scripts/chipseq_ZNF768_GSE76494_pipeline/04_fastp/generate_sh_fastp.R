...
fastq_folder <- "chipseq_ZNF768_GSE76496"
...
for (sample_name_i in sample_name_list) {
  ...
  fastq_folder_i <- fastq_folder
  # message("# ", i, " | ", sample_name)
  
  ##### R1 & R2
  # message(" > R1 & R2")
  in_fastq_R1_filename <- "HEK293_ZNF768_rep1.fastq.gz" #df$fastq_R1_filename[i]
  ...
  out_fastq_R1_filename <- paste0(sample_name, ".fastq.gz")
  ...
  file_sh <- file.path("scripts", script_pipeline_dir, "04_fastp/batch_sh",
