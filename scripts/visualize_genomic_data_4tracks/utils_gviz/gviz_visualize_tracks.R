...
##### HEK293
# read HEK293 bigwig
readBigwig_HEK293 <- function(custom_params) {
  chrom <- custom_params$chrom
  chromstart <- custom_params$chromstart
  chromend <- custom_params$chromend
  message("# Reading HEK293 signals at ", paste0("chr", chrom), ":", chromstart, "-", chromend)
  
  #
  bw_dir <- "/Users/chris/Desktop/20250221_ZNF768_project/output/chip-pipeline_ZNF768_GSE76496-GRCh38_SE/tracks_byReplicate"
  HEK293_signal <- list()
  bw_filename <- "HEK293_ZNF768_rep1_RPKM.bw"
  bw_filepath <- file.path(bw_dir, bw_filename)
  bw_signal <- readBigwig(file = bw_filepath, params = custom_params)
  HEK293_signal <- bw_signal
}

##### HEPG2
# read HEPG2 bigwig
readBigwig_HEPG2 <- function(custom_params) {
  chrom <- custom_params$chrom
  chromstart <- custom_params$chromstart
  chromend <- custom_params$chromend
  message("# Reading HEPG2 signals at ", paste0("chr", chrom), ":", chromstart, "-", chromend)
  
  #
  bw_dir <- "/Users/chris/Desktop/20250221_ZNF768_project/output/chip-pipeline_ZNF768_ENCSR181ABP-GRCh38_SE/tracks_pooled"
  HEPG2_signal <- list()
  bw_filename <- "HEPG2_ZNF768_pooled_RPKM.bw"
  bw_filepath <- file.path(bw_dir, bw_filename)
  bw_signal <- readBigwig(file = bw_filepath, params = custom_params)
  HEPG2_signal <- bw_signal
}
