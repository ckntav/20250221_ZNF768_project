...
plotSignal(HEPG2_bw, params = params_i,
           y = "0.1b", height = height_track, scale = TRUE,
           range = c(0, HEPG2_maxRange),
           # ymax = scalefactor_i,
           linecolor = "#4A9E7B", fill = "#4A9E7B",
           # label = "U2OS",
           fontsize = fontsize_val)

plotText(
  label = "HEPG2 ZNF768 (pooled)", fontsize = 3, fontcolor = "#4A9E7B",
  x = width_val - 0.5, y = 0.3 + height_track*3 + 0.1*3, just = c("right", "top"),
  default.units = "inches")
