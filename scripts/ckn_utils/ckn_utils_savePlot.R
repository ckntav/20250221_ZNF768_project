...
#####
savePlotTIFF <- function(plot, output_dir, output_file, width_val = 25, height_val = 22, res_val = 1200) {
  system(paste("mkdir", "-p", output_dir))
  output_filepath <- file.path(output_dir, paste0(output_file, ".tiff"))
  tiff(file = output_filepath, width = width_val, height = height_val, units = "in", res = res_val)
  print(plot)
  dev.off()
  message(" > Plot (tiff) saved in ", output_filepath)
}
